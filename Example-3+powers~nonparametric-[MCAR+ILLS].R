...
## Asymptotic Power Study of 6 test statistics under nonparametric regression ##

## Missing Completely at Random (MCAR) & ILLS method of estimation of m(X) ##

## Ex - 3 : X~N(0,1) and e|X=x =D= {(W_x-r_x)/(10*sqrt(2*r_x))} with W_x ~ chi_square_{r_x}, r_x = (1/3x)  ##
...
x = rtruncnorm(n,0,(1/3),0,1)  ## generation of 100 i,i.d. covariate values under H0 ##
...
for(i in 1:n)
{
  df.x[i]<- round(1/(3*x.sort[i]),0)
}

e.x<- c()  ## conditional error observations of size n under H1 ##
for(i in 1:n)
{
  e.x[i]<- (rchisq(1,df.x[i],0)-df.x[i])/(10*sqrt(2*df.x[i]))
}
...
for(i in 1:n)
{
  h.ast[i]<- dchisq(e.x[i],df.x[i],0)*10*sqrt(2*df.x[i])/(dtruncnorm(x.sort[i],0,(1/3),0,1)*dnorm(e.sort[i],0,1))
}
...
plot(gm, KS, xlim=c(0,10), ylim=c(0,1), type="l", pch=10, col="red", xlab=expression(gamma), ylab=expression("Nonparametric Powers (MCAR & ILLS)"))
# Add a line
