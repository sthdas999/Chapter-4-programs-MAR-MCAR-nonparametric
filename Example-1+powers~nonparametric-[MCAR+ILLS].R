...
x = rtruncnorm(n, -1/5, Inf, 0, 1)  ## generation of 100 i,i.d. covariate values under H0 ##
...
M2<- function(v)  
{
  ...
  for(i in 1:length(x.dash))
  {
    vec2[i]<- (v-x.dash[i])*ep.kernel((v-x.dash[i])/h)
  }
  return(sum(vec2))
}
...
M2.1<- function(v)  
{
  ...
  for(i in 1:n)
  {
    v2[i]<- (v-x[i])^2*ep.kernel((v-x[i])/h)
  }
  return(sum(v2))
}
...
for(i in 1:n) 
{
  e.x[i]<- rnorm(1, 0, sqrt((1+5*x.sort[i])/100))
}

h1<- c()  ## computation of the ratio h for contiguous alternatives ## ##
for(i in 1:n)
{
  h1[i]<- round(sqrt(100/(1+5*x.sort[i]))*exp(0.5*e.x[i]^2*(1-(100/(1+5*x.sort[i])))),4)-1
}
